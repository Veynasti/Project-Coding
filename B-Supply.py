    ...
    while True:
        ...
        try:
            ...
            if akunUser == 1:
                ...
                time.sleep(2)
                continue #mengulang perulangan sampai inputan dari user benar maka dia bisa melanjutkan ke fungsi selanjutnya
        except ValueError:
...
def login_admin():
    ...
    print('='*40)
    inputUser = input("Masukkan username Anda: ").rstrip().lower()  #lower untuk membuat inputannya itu otomatis kecil
    inputPass = input('Masukkan password Anda: ').rstrip().lower()
    data = pd.read_csv('csv/dataAdmin.csv')
    user = data[(data['Username'] == inputUser) & (data['Password'] == inputPass)]  #di sini untk mengecek apakah inputan dari user ada di data kolom (username) dan inputan password ada di data kolom(password)
    if not user.empty:  #jika tidak user kosong
        print("\nSelamat datang, Anda berhasil login sebagai Admin...")
        time.sleep(2)   #untuk memberhentikan program selama 2 detik 
        halaman_admin()    
    else:
...
def login_user():
    ...
    print("=" *40)
    userInputh = input("Masukkan username: ").rstrip().lower()  #rstrip untuk menghapus spasi di sisi kanan
    ...
    if not user.empty: 
        ...
        os.system('cls')
        print(f'Pengguna dengan username [{userInputh}], Password [{userPassh}] tidak ditemukan.')  #format untuk memformat string dengan memasukkan nilai ke dalam template format
        print("Ingin mendaftar atau mencoba lagi?")
        print('-'*40)
        print("[Enter] untuk mencoba lagi")
        print("[y] untuk mendaftar")           
        jawaban = input("Masukkan pilihan >>> ").strip().lower()    #strip untuk menghapus di kedua sisi
        if jawaban == 'y':
            register()
        elif jawaban == '':
            login_user()
...
def lihat_profil():
    ...
    print('='*40)
    data = pd.read_csv('csv/dataUser.csv')
    cek = data.loc[data['Username'] == userInputh]  #loc yaitu untuk mengakses data, apakah nilai dari inputan sesuai dengan kolom di csv atau tidak
    saldo = cek['Saldo'].values[0]
    tanggal = cek['Tanggal Daftar'].values[0]   #values [0] untuk mengakses nilainya
...
def pemesanan():
    ...
    data.index = range(1, len(data)+1)  #fungsi len untuk menghitung jumlah baris
...
def pesan():
    os.system('cls')
    data = pd.read_csv(f'csv/toko/{kode}.csv')  #untuk menyocokkan kode toko sesuai dengan inputan user
    dataUser = pd.read_csv('csv/dataUser.csv')  #membaca data user yang akan membeli barang
    data.index = range(1,len(data)+1)
    print(tabulate(data,headers='keys',tablefmt='grid'))
    kodebrg = input('Masukkan kode barang yang akan dibeli : ').upper()
    #pengecekan stok brang dan saldo
    if kodebrg in data['KodeBrg'].values:
        cek = data[data['KodeBrg'] == kodebrg]
        cekTrue = cek['Stok'].values[0] #supaya tidak muncul dalam bentuk list
        cek2 = dataUser[dataUser['Username'] == userInputh]
        cekSaldo = cek2['Saldo'].values[0]
        while True: #untuk mengulang terus sampe inputannya benar
            try:
        ...
        if cekTrue < 1:
            ...
            time.sleep(2)
            pesan()
        elif cekSaldo < a * cek['Harga'].values[0]: #apakah saldo bisa membeli barang tersebut
            print('Saldo tidak mecukupi untuk Membeli...')
            time.sleep(2)
            pesan()
        else:
            cek1 = data[data['KodeBrg'] == kodebrg] #memfilter data
            #mengambil informasi barang untuk dikaitkan dengan atur pengiriman
            cekHarga = cek1['Harga'].values[0]  #mendapatkan harga brg
            cekNama = data['NamaBrg'].values[0] #mendapatkan nama brg
            total = cekHarga * a    
            hari = datetime.date.today()
            randomAngka = random.randint(1,999) #menghasilkan angka acak 1-999 sebagai kode pengiriman
            data.loc[data['KodeBrg'] == kodebrg, 'Stok'] -= a
            dataUser.loc[dataUser['Username'] == userInputh, 'Saldo'] -= (total)  #dikurangi dengan total harga pembelian
            data.to_csv(f'csv/toko/{kode}.csv',index=False) #perubahan pada stok barang disimpan ke csv
            dataUser.to_csv('csv/dataUser.csv',index=False) #perubahan saldo juga disimpan ke csv
            #menyimpan data transaksi ke csv
            if not os.path.exists(f'csv/histori/{userInputh}.csv'): #os.path.exists () digunakan untuk mengecek apakah file sudah ada atau belum
                with open(f'csv/histori/{userInputh}.csv','w',newline='') as file:
                    writer = csv.writer(file)
                    writer.writerow(['kode','NamaBrg','Jumlah','Harga','Tanggal'])  #MENULISKAN HEAD
                with open(f'csv/histori/{userInputh}.csv','a',newline='') as file:
                    writer = csv.writer(file)
                    writer.writerow([kodebrg,cekNama,a,cekHarga * a,hari])  #menuliskan isi datanya
            #untuk mengatasi saat file sudah ada di csv
            elif os.path.exists(f'csv/histori/{userInputh}.csv'):
                statusKirim = 'Belum dikirim'
                with open(f'csv/histori/{userInputh}.csv','a',newline='') as file:
                    writer = csv.writer(file)
                    writer.writerow([kodebrg,cekNama,a,total,hari]) #langsung menuliskan isi datanya
                with open('csv/dataPengiriman.csv', 'a', newline='') as kirim:
                    writer = csv.writer(kirim)
                    writer.writerow([randomAngka,userInputh,hari,total,statusKirim])    #langsung menuliskan isi datanya
...
def cek_harga():
    ...
    if a in data['kode'].values:
        os.system('cls')
        dataMitra = pd.read_csv(f'csv/toko/{a}.csv')    #{a} membuka kode toko sesuai dengan kode yang diinputkan
...
def keluhan():
    ...
    with open('csv/keluhanUser.csv', 'a', newline='') as file:
        writer = csv.writer(file)
        writer.writerow([userInputh,keluhUser,hari])    #methid writerow untuk menulis satu baris data ke file
...
def atur_pengiriman():
    while True:
        ...
        data['kode'] = data['kode'].astype(int) #karena data di csv disimpan dlm bentuk string maka disini dirubah ke tipe data int supaya selaras dengan inputan
        data.index = range(1, len(data) + 1)
        print(tabulate(data, headers='keys', tablefmt='grid'))
        ...
        try:
            ...
            print('Pilihan harus angka & tidak boleh kosong!')
            time.sleep(2)
            continue   #untuk melooping kembali ke awal perulangan sampai inputan benar,ketika benar bisa melanjutkan ke program selanjutnya

        if inputUser == 0:
            halaman_admin()
            return  #jika terpenuhi maka akan meberhentikan program ini lalu keluar dari program ini

        if not data[(data['kode'] == inputUser) & (data['status'] == 'Diantar')].empty: #mengecek apakah di kolom kode ada data yang sesuai dengan inputan user,dikolom status bernilai diantar,empty mengembalikan nilai True jika benar kosong
            print('Pesanan dengan kode tersebut telah diantar!')
            time.sleep(2)
            continue

        if inputUser in data['kode'].values:    #jikak kode inputan user ada di kolom kode maka akan melanjutkan ke pemilihan kendaraan
            os.system('cls')
            ...
            if not data2[(data2['kode'] == inputKode) & (data2['Status'] == 'BEROPERASI')].empty:
                ...
                time.sleep(2)
                continue
            #program untuk merubah status kendaraan
            if inputKode in data2['kode'].values:   #merubah status di csv kendaraan
...
def kelola_barang():
    ...
    if ((data['kode'] == inputToko) & (data['Status'] == 'Tersedia')).any():    #method any() digunakan untuk mengecek apakah ada salah satu nilai True dalam array
...
def hapus_barang():
    ...
    if kode in data['KodeBrg'].values:
        data = data[data['KodeBrg'] != kode]    #mempertahankan data yang tidak sama dengan kode yang diinput,data yang sama akan dihapus
...
def hapus_kendaraan():
    ...
    if kode in data['kode'].values:
        data = data[data['kode'] != kode]   #fungsi != untuk mempertahankan data yang tidak sama dengan kode yang diinput, sehingga data yang diinput akan terhapus
        data.to_csv('csv/dataKendaraan.csv', index=False)   #proses penyimpanan data yang telah diubah
...
def ganti_pass_user():
    ...
    if user in data['Username'].values:
        ...
        data.loc[data['Username'] == user, 'Password'] = user2  #data.loc yaitu digunakan untuk mengakses ketika ingin mengubah data
        data.to_csv('csv/dataUser.csv', index=False)
...
def hapus_user():
    ...
    if user in data['Username'].values:
        data = data.loc[data['Username'] != user]   #!= Menggunakan filter tersebut untuk memilih baris dengan kondisi True (baris yang tidak memiliki username yang sama dengan user).
# Akibatnya, baris dengan username yang cocok akan dihapus dari DataFrame.
        data.to_csv('csv/dataUser.csv', index=False)
